...
from segmenter.segmentor import WordSegmenter
...
def csv_to_dataset(filename, content_name, label_name='', preprocess=True, tokenize=None, augmentations=[]):
  ...
  if preprocess:
    X = np.array(list(map(normalize, texts)), dtype=object)
    # X = np.vectorize(normalize)(X)
  if token is not None:
    X = np.array(list(map(WordSegmenter.tokenizer(tokenize), texts), dtype=object)
